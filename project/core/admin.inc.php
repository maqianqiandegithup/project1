...
function getAllAdmin(){   
    $sql="select id,username,email from jd_admin ";
    $rows=fetchAll($sql);
    return $rows;
}
function getAdminByPage($page,$pageSize=2){
    $sql="select * from JD_admin";
    global $totalRows;
    ...
    $sql="select id,username,email from jd_admin limit {$offset},{$pageSize}";
    $rows=fetchAll($sql);
    return $rows;
}
/**
...
function logout(){
    ...
    if(isset($_COOKIE[session_name()])){
        setcookie(session_name(),"",time()-1); //删除cookies
    }
 ...
 */
function delUser($id){
    $sql="select face from jd_user where id=".$id;
    $row=fetchOne($sql);
    $face=$row['face'];
    if(file_exists("../uploads/".$face)){
        unlink("../uploads/".$face);
    }
    if(delete("jd_user","id={$id}")){
